...
<body>
    ...
    "zone": "main|side"                   // zone to change (optional, default "main")
            ...
            <p>For <code>add</code>, the endpoint enforces the deck format's copy limit across the main deck and sideboard combined. The limit includes card-specific exceptions and is unlimited for unrestricted formats. An add that exceeds the limit returns HTTP 400 with <code>error: "Copy limit exceeded"</code>, <code>cardID</code>, <code>existingCopies</code>, <code>requestedCount</code>, and <code>maxCopies</code>.</p>
    ...
    "cards": [                              // array of changes (max 200 per request)
        {
            ...
            <p>The final batch result must respect the deck format's copy limit across the main deck and sideboard combined. This includes card-specific exceptions and unrestricted formats. A batch that increases a card beyond its limit returns HTTP 400 without writing, with <code>error: "Copy limit exceeded"</code>, <code>cardID</code>, <code>existingCopies</code>, <code>resultingCopies</code>, and <code>maxCopies</code>. An existing over-limit deck can still have that card reduced or edited without increasing its total.</p>
