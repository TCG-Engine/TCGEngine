...
- **Retro log** — `.Codex/skills/swudeck-session-close/references/lessons-learned.md`. Skim the
...
Per the AGENTS.md engineering rule, SWUDeck UI/CSS changes must be checked in **Chromium +
...
- Test login is `Drixx` / `pass` (AGENTS.md `## Creds`); the users table username column is
