...
```
python3 .Codex/skills/blast-radius/scripts/blast-radius.py
```
...
```
python3 .Codex/skills/blast-radius/scripts/blast-radius.py --base origin/main --head HEAD
python3 .Codex/skills/blast-radius/scripts/blast-radius.py --base HEAD~5 --head HEAD
```
...
```
python3 .Codex/skills/blast-radius/scripts/blast-radius.py swusim
python3 .Codex/skills/blast-radius/scripts/blast-radius.py azukideck --base origin/main --head HEAD
```
...
`.Codex/skills/blast-radius/output/blast-radius[-<root>]-<timestamp>.md`
...
2. Classifies each changed file:
   ...
   - **Public API surface** — anything under `APIs/`, or `Stats/APIs.php` /
     `Stats/*API.php`. Flags the AGENTS.md rule: verify the change is
...
3. Prints a suggested check per affected product: SWUSim has an automated
   ...
   suggestion is a manual playtest using the test logins in AGENTS.md.
...
- **Public API surface touched** → stop and apply the AGENTS.md rule before
