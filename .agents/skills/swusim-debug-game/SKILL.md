...
`.Codex/SWUSim/refs/card-specific-rulings.md` is the official card-database clarification list
...
grep -n -A 6 "^### Boba Fett" .Codex/SWUSim/refs/card-specific-rulings.md
...
```bash
docker exec -w /var/www/html/TCGEngine swustats-swusim-web-server-1 \
  php -d xdebug.mode=off .Codex/skills/swusim-debug-game/scripts/run-schema-tests.php \
...
**Use `.Codex/skills/swusim-debug-game/scripts/run-schema-tests.php`, run via the container's PHP.** It clones `zzRegressionSWUSim.php`'s exact environment. Two naïve alternatives lie:
...
- **Container:** `swustats-swusim-web-server-1`, web root `/var/www/html/TCGEngine` (the repo is mounted there, `.Codex/` included).
