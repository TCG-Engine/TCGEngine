...
status) + the lean `.Codex/SWUSim/instructions.md` orientation, then proceed.

## Flow (overview)

```
Stage 1  scan + classify inventory  +  Stage 0.5 core-mechanic survey
         -> write scratch .Codex/tmp/<set>-inventory.md  -> STOP for human review
...
3. **Confirm via the CR:** look the term up in `.Codex/SWUSim/refs/comprehensive-rules.md`
   — and cross-check `.Codex/SWUSim/refs/card-specific-rulings.md` (official card-database
   ...
   (and `.Codex/SWUSim/refs/game-refs.md`). Only a term the CR defines as a mechanic is a
   real blocker.
...
Write `.Codex/tmp/<set>-inventory.md` (scratch; deletable after Stage 2):
