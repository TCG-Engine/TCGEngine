...
function FaBWTRHeroActive(int $player): bool {
    foreach(GetHero($player) as $iarHero)if(in_array('IAR_WIND_LOCK',(array)($iarHero->TurnEffects??[]),true))return false;
    if(FaBMPGEffects($player,'BLIND'))return false;
    $hero=GetHero($player)[0]??null;if($hero&&FaBSUPSuppressed($hero))return false;
    foreach(GetHero($player) as $o)if(empty($o->removed)&&(in_array('NO_ABILITIES',(array)($o->TurnEffects??[]),true)||!empty(FaBObjectCounters($o)['_overrides']['NO_ABILITIES'])))return false;
...
function FaBWTRTag(object $obj, string $tag): void {
    $tag=FaBPENPowerTag($obj,FaBHVYPowerTag($obj,FaBOUTPowerTag($obj,$tag)));if($tag==='')return;
...
function FaBWTRCanPlay(int $player, array $found, array $state): bool {
    ...
    $mandatoryDiscard = ['alpha_rampage','bloodrush_bellow','breakneck_battery','primeval_bellow','reckless_swing','savage_feast','savage_swing','wrecker_romp','barraging_big_horn','swing_fist_think_later'];
    ...
    if (FaBHasType($id, 'Attack Reaction') && $state['window'] === 'REACTION') {
        ...
        if($base==='concealed_blade')return FaBOUTTargets($player,'hybrid')!=='';
        if(in_array($base,['spreading_plague','knives_out'],true))return true;
        if(in_array($base,['cut_to_the_chase','shred'],true))return FaBDYNAttacks($player,$base==='shred'?'ASSASSIN':'CONTRACT')!=='';
        if($base==='incision')return FaBHasType($obj,'Dagger');
        if($base==='puncture')return FaBDYNAttacks($player,'BLADE')!=='';
        if ($base === 'lunging_press') return FaBWTRIsAttackAction($obj);
        if ($base === 'twinning_blade') return count(FaBChoiceRefs($player,'Weapons',['type'=>'Sword']))>0;
...
function FaBWTRCardPitched(int $player, string $cardID): void {
    FaBUPRPitched($player,$cardID);
    FaBOUTPitch($player,$cardID);
    if ($cardID !== 'heart_of_fyendal_blue') return;
    foreach (FaBOpponents($player) as $opponent) if (FaBPENLifeMore($opponent,$player)) { FaBCRUGainLife($player,1); break; }
}
...
function FaBWTRPayAdditionalCosts(int $player, object $stackObj): void {
    $base = FaBWTRBase($stackObj->CardID); $mandatory = ['madcap_charger','madcap_muscle','savage_beatdown','alpha_rampage','bloodrush_bellow','breakneck_battery','primeval_bellow','reckless_swing','savage_feast','savage_swing','wrecker_romp','barraging_big_horn','swing_fist_think_later'];
...
function FaBWTRCardPlayed(int $player, string $mzID, string $cardID, string $fromZone): void {
    ...
    foreach ($effects as $effect) {
        ...
        elseif ($type === 'SUP_NEXT_AA') $applies = FaBWTRIsAttackAction($obj);
        ...
        foreach(['monDusk'=>'MON_DUSK_HIT','monPlow'=>'MON_PLOW','monSoul'=>'MON_SOUL_HIT','monBottom'=>'MON_BOTTOM_HIT','monPuppet'=>'MON_PUPPET_HIT'] as $key=>$tag)if(!empty($effect[$key]))FaBWTRTag($obj,$tag);
        if (!empty($effect['nature'])) FaBWTRTag($obj, 'WTR_NATURE_HIT');
    }
    FaBWTRSetEffects($player, $remaining);

    FaBHNTPlayed($player,$obj,$fromZone);FaBSEAPlayed($player,$obj,$fromZone);FaBMPGPlayed($player,$obj,$fromZone);FaBSUPPlayed($player,$obj,$fromZone);FaBPENPlayed($player,$obj,$fromZone);
    FaBFaiCardPlayed($player,$obj);
    FaBCRUCardPlayed($player,$obj,$fromZone);
    FaBMONCardPlayed($player,$obj,$fromZone);
    FaBELEPlayed($player,$obj,$fromZone);
    ...
    if ($base === 'scar_for_a_scar') foreach (FaBOpponents($player) as $opponent) if (FaBPENLifeMore($opponent,$player)) {
        FaBWTRTag($obj, 'GO_AGAIN'); break;
    }
    if ($base === 'wounded_bull') foreach (FaBOpponents($player) as $opponent) if (FaBPENLifeMore($opponent,$player)) {
        FaBWTRTag($obj, 'WTR_WOUNDED_BULL'); break;
    }
    if ($cardID === 'last_ditch_effort_blue' && count(FaBChoiceRefs($player,'Deck')) === 0) { FaBCRUSelfTag($obj, 'WTR_POWER:4'); FaBWTRTag($obj, 'GO_AGAIN'); }
    if (in_array('WTR_NIMBLE_PAID',(array)($obj->TurnEffects??[]),true)){FaBCRUSelfTag($obj,'WTR_POWER:1');FaBWTRTag($obj,'GO_AGAIN');}
...
function FaBWTRResolveCard(int $player, object $source, ?object $resolved): void {
    ...
    if ($base === 'sigil_of_solace') { FaBCRUGainLife($player,FaBWTRPitchValue($id,[3,2,1])); return; }
    if ($id === 'tome_of_fyendal_yellow') { DoDrawCard($player,2); if(($source->SourceZone??'')==='Arsenal') FaBCRUGainLife($player,FaBHandCount($player)); return; }
...
function FaBWTRResolveReaction(int $player, object $source): void {
    ...
    if($amount)FaBCRUSelfTag($attack['object'],'WTR_POWER:'.$amount);
...
function FaBWTRAttackDeclared(int $player, object $attack, int $defender): void {
    ...
    if(isset($combo[$base])){[$required,$power,$go,$dom]=$combo[$base];if(FaBWTRBase((string)$state['previousAttackCardID'])===$required){if($power)FaBCRUSelfTag($attack,'WTR_POWER:'.$power);if($go)FaBWTRTag($attack,'GO_AGAIN');if($dom)FaBWTRTag($attack,'DOMINATE');if($base==='hurricane_technique')FaBWTRTag($attack,'WTR_RETURN_HAND');}}
...
function FaBWTRAttackPowerModifier(int $player, object $attack, array $state): int {
    ...
    if(!FaBCRUCount($player,'SNAG')&&$base==='barraging_brawnhide'&&FaBWTRNonEquipmentBlockCount($state)<2)$delta++;
    if(!FaBCRUCount($player,'SNAG')&&$base==='fluster_fist'&&FaBWTRBase((string)$state['previousAttackCardID'])==='open_the_center')$delta+=intval($state['chainHits']??0);
...
function FaBWTRAttackHasGoAgain(array $state, object $attack): bool {
    ...
    if(FaBCRUGoAgain(intval($state['attacker']),$attack,$state))return true;
...
function FaBWTRApplyNextDefense(int $player, object $obj): void {
    if(FaBHasType($obj,'Action')&&FaBELECount($player,'NEXT_DEFENSE')){FaBWTRTag($obj,'WTR_DEFENSE:'.FaBELECount($player,'NEXT_DEFENSE'));FaBELEClear($player,'NEXT_DEFENSE');}
    $remaining=[];
...
function FaBWTRHit(int $player, object $attack, int $amount): void {
    ...
    FaBSetState($state);
    if(FaBCRUHitSuppressed($attack,true))return;
    if(!FaBCRUHitSuppressed($attack)){
    $base=FaBWTRBase($attack->CardID);
...
function FaBWTRIntellectModifier(int $player):int{$evo=FaBSEACount($player,'INTELLECT')+FaBROSIntellect($player)+FaBEVOEffect($player,'INTELLECT')+FaBHVYCount($player,'BASE_INTELLECT');foreach(FaBLiveSeats() as $seat)$evo-=count(FaBMONArena($seat,'contest_the_mindfield'));foreach(FaBWTREffects($player)as$effect)if(($effect['type']??'')==='INTELLECT')return $evo+intval($effect['amount']);return $evo;}
...
function FaBWTRCanActivate(int $player,string $mzID):bool {
    ...
    if(FaBSUPBaitLocked($player,$f['object']))return false;
    ...
    if(FaBSEAWeaponCanAttack($player,$f))return true;
...
function FaBWTRActivate(int $player,string $mzID,?int $index=null):bool {
    if(!FaBWTRCanActivate($player,$mzID))return false;
    $f=FaBIdentityFromMZ($mzID);$o=$f['object'];$id=$o->CardID;
    if(FaBSEAWeaponCanAttack($player,$f))return FaBSEAWeaponAttack($player,$f);
    if(FaBDYNWeaponCanAttack($player,$f))return FaBDYNWeaponAttack($player,$f);
    if(FaBMONArenaCanAttack($player,$f))return FaBMONArenaAttack($player,$f);
    $actions=FaBARCAbilityActions($player,$f);
    if($actions){$index??=array_key_first($actions);return isset($actions[$index])&&FaBARCActivate($player,$f,$index);}
    ...
    $spec=FaBWTRAbilitySpec($id);return $spec!==null&&FaBWTRAnnounceAbility($player,$f,$spec);
}

function FaBWTRStartTurn(int $player):void {
    FaBMPGBegin($player);FaBSEABegin($player);
    ...
    foreach(GetArena($player)as$o){
        if(!is_object($o)||!empty($o->removed)||HasNoAbilities($o))continue;
