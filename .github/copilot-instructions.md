...
- A lobby opts in with `$lobby->casterMode = true`; `Core/GameAuth.php` stores the authoritative flag, seat keys, spectator key, and serialized gamestate together in one server-only APCu envelope. A small derived auth lookup may be recreated from that envelope for polling performance; losing the lookup never invalidates a seat. Keeping authoritative auth and state in the same cache entry prevents cache pressure from leaving a live gamestate with separately evicted credentials. The envelope is intentionally ephemeral and missing auth must fail closed for lobby-managed sim roots in hosted environments; local loopback requests and explicit `DEVENV=true` development environments bypass sim auth so persistent test games survive APCu restarts. Asset editors such as AzukiDeck and SWUDeck do not use lobby auth.
