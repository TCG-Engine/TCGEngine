...
- [x] Add the attachment layer behind "Attach to …" assets: an `AttachedTo` field on the Assets zone holding the host's UniqueID, a shared `HellbreakBeginAttach`/`HellbreakAssetIsAttachedTo`/`HellbreakDetachAssetsFromHost` helper family, and detach-to-crypt wired into the shared minion-kill funnel. Pools follow the rulebook: "character" is any minion or monster, and an unqualified pool spans both seats. Proved with Vampire Fangs (DOT_171).
- [ ] Implement the nine remaining attachment cards, each of which needs more than the static grant Vampire Fangs proved: DOT_047 and DOT_133 (a "when attached" trigger), DOT_050 and DOT_211 (stat grants), DOT_091 and DOT_131 (granting an `Attack —` ability to the host), DOT_095 ("can't ready", minion pool), DOT_130 (location pool), DOT_212 (damage prevention on the host).
- [ ] Drive an attach through a live game fixture. The unit suite covers the helpers and the grants, but `HellbreakBeginAttach` → `MZCHOOSE` → `HellbreakResolveAttach` has not run in a real game; no fixture deck contains an attach card, and DOT_171 is Revenant loyalty while both GAMA demo decks are Cursed/Deranged.
- [ ] Add a `RefreshReady`-style dispatch point for "when this card is discarded from your hand or deck" (DOT_157, DOT_176) — no macro currently fires at that moment.
- [x] Resolve the missing Marishka source image/rules: her face was recovered from the HellbreakHub mirror, transcribed into `ReviewedCardFaces.json` (icons verified against cards with known values), and implemented as a `PlayCostModifier` plus the `Attack / Scheme` halves, on shared `HellbreakDiscardTopOfDeck` / `HellbreakCollectCardResourceIcons` helpers.
- [x] Back-fill card tests for every implemented GAMA-deck card (35 of them) across four suites — value modifiers, `Played`, `Jumpscare`, and listeners/scheme/activated. Each implementation was also read against its printed text; all matched. Coverage before this pass was 2 cards of 49.
- [x] Link the four unlinked printings (DOT_273, DOT_433, DOT_439, DOT_440) in `baseCards`. DOT_273 was a live bug: the borderless Dr. Seward resolved to nothing and played with no abilities. Base-card count is now 172, not 176.
- [x] Prove engine DISPATCH for `InitiativeAssigned` and `Played` with the `dispatch-macro` integration fixture — a real game through setup, bidding, initiative and a card play, asserting that Dracula's initiative listener and the Bat's Played macro both fired. Added a `scalar_zone_equals` assertion type; nothing could previously assert a resource value, which is most of Hellbreak's state.
- [ ] Extend dispatch coverage to the remaining macro families: `JumpscareUsed`, `LocationTaken`, `DamageDealt`, `SchemeStarted`, `ActivateAbility`.
- [ ] Back-fill card tests for the 8 implemented non-GAMA cards: DOT_023, DOT_031, DOT_041, DOT_088, DOT_129, DOT_160, DOT_174, DOT_209.
- [x] Add the turn-effect system: temporary stat changes, granted keywords/traits and markers, stored in the existing `TurnEffects` field on all four card-bearing zones. Ported from SWUSim with its known failure modes fixed — an unregistered base still expires, every bearing zone is swept, a stale effect stops applying on READ even if no sweep ran, and identical applications stack instead of de-duping. Unblocks the 23 cards whose text says "this attack", "this phase" or "this round".
- [x] Implement Orca, Timeworn Trawler (DOT_092): Lurking Action, pay 1 malice and exhaust, nested attack via `HellbreakChooseAttacker`, `+2 combat this attack` as an attack-duration turn effect, and the Sailor blood rider.
- [x] Implement Shark in the Pond (DOT_136): the set's only opponent-decision-inside-your-ability card. `await $opponent.YesNo(...)` queues for the other seat; added `HellbreakPlayMinionFromHandForFree` and `HellbreakHandMinionTargets` for the free Shark. **Both GAMA demo decks are now card-complete (37 implemented, 3 vanilla/keyword-only).**
- [ ] Playtest the GAMA decks end to end. Card coverage is complete and unit-tested, but the universal rules are only exercised by one integration fixture; real games are what will surface sequencing problems.- [x] Prefer the better card-art source per card: `import-research-art.php` fills gaps from the HellbreakHub mirror and upgrades a card only when the hub source is higher resolution, with orientation treated as correctness. Wired to the workbook-refresh checkbox.
