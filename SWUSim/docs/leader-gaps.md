...
> **♻ RESTORED 2026-08-27.** This file was deleted incidentally in `46b89e5e` ("feat(SWU): UI updates" —
> 160 deletions, nothing else in that commit touched it). It is still cited as the baseline gap list by
> BOTH `swusim-implement-card` and `swusim-set-validation`, so a session following either skill was being
> sent to a file that no longer existed. Restored verbatim; the resolution note below still stands.
