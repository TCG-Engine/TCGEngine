...
<!-- ⚠ HMW_011 Darth Sidious is Done for PREMIER (2-player) — 11/11 green, including the new
     "when you deal 4+ damage" observer wired into all five damage funnels. Its TWIN SUNS behaviour
     is UNVERIFIED: the reaction offer built in a 4-seat game returns only [myBase-0 & p2Base-0]
     (no seat-3/4 bases, no units at all). Parked repro + full diagnosis:
     SWUSim/docs/pending-repros/HMW_011_TwinSuns_OfferPool.md — suspected pre-existing fan-out issue
     that this card is the first to surface, NOT card logic. -->
