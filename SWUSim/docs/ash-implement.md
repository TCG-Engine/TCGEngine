...
**CLEARED — ASH_155** Grogu ("When you take the initiative: you may attack with a unit"): hooked in `SWUTakeInitiative` (one `SWUQueueMayChooseTarget` offer per controlled Grogu over ready units → `ASH_155#0` continuation → `BeginSWUAttack`). The bonus attack is out-of-sequence (the initiative pass already swapped the turn), so its terminal after-action must not end the action a second time. **This used to be a one-shot `SWU_SUPPRESS_AFTERACTION` flag consumed in `SWUAfterAction`; the flag was removed 2026-08-31** and the continuation now sets nothing — `SWUPassAction` stamps the initiative claim's action id closed, so the close ledger refuses the duplicate for every card rather than for this one by hand (see `action-close-deferrals.md` §1). The feared 5-combat-site conflict was a non-issue: the inline combat after-actions are already `_SWUInTriggerResumeMode()`-gated, so only the single terminal after-action needed suppression. 3 tests (accept+turn-integrity via a P2 follow-up action, decline, no-ready-unit fizzle).
