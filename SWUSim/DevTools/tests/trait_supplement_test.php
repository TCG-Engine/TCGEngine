...
$n = SWUApplyTraitSupplement($traits, $fixture);
...
$traits2 = [];
SWUApplyTraitSupplement($traits2, $fixture);
...
check(strpos($gen, 'SWUApplyTraitSupplement') !== false, 'generator applies the trait supplement');
// ...and applies it for SWUDeck too, not only SWUSim. It was gated on `$rootName == "SWUSim"`,
// which left all 91 bases in SWUDeck's dictionary with an empty trait list.
check(preg_match('/if\\(\\$rootName == "SWUSim"\\)\\s*\\{\\s*require_once __DIR__ \\. \x27\\/AppCore/', $gen) !== 1,
      'trait supplement is NOT gated to SWUSim alone');
check(strpos($gen, 'SWUDeck') !== false && preg_match('/\\$rootName == "SWUSim" \\|\\| \\$rootName == "SWUDeck"[\\s\\S]{0,200}SWUApplyTraitSupplement/', $gen) === 1,
      'generator applies the supplement for BOTH SWUSim and SWUDeck');
