...
#// so nothing can be played. Leader exhausts; the Raid unit stays in hand. Ref: "does not allow playing a
...
#// stays in hand. Ref: "does not allow playing an event that shares a Keyword with the chosen unit".
...
#// attacked this phase" and the ability does nothing (the Raid unit in hand stays). Ref: "does not work if
...
#// Ref: "the leader side ability can still be used if the discounted cost is 0".
...
#// it costs the full 1 (5 -> 4). Ref: "the effect does not give a discount if the played unit does not share
...
#// yet it is an EVENT so no discount applies (5 -> 2, not 3). Ref: "the effect does not give a discount for
#// event cards".
...
#// aspects covered) gets no discount (6 -> 1, not 2). Ref: "the discount does not apply to units that only
...
#// resources. Ref: "the leader unit side ability can still be used if the discounted cost is 0".
