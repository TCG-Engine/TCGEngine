...
#// (SOR_046 3, LAW_124 2, SOR_128 1) — the 1-HP SOR_128 is defeated, leaving two damaged units. (FT:
#// "should distribute damage among targets when defeated".)
...
#// attacks the 3/3 (SEC_080) and dies to the counter; the lone survivor SOR_046 (3/7) takes all 6. (FT:
#// "should automatically deal all damage to that target"; SWUSim still surfaces the distribute prompt, so the
...
#// ability has no legal target and simply fizzles. (FT: "should do nothing if there are no enemy units".)
