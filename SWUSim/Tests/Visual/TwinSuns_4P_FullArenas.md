...
#   • Two-level nav (same as 3-player): HOME (you vs everyone) shows a mini-board preview per opponent
#     (P2/P3/P4) replacing the opponent board region; click a preview → that you-vs-one matchup with a
#     "Go back" button. No carousel arrows, no opp-vs-opp view (egocentric — every view's YOU at bottom).
#   • Each preview tile is backed by that seat's key-art playmat (P2 JTL, P3 LAW, P4 SHD in the editor).
#   • Concat leaders: every seat (P1–P4) shows TWO leaders side-by-side over the base.
#   • Each seat has 2 ground + 2 space units (one damaged in each arena for the damage counter).
#   • Switching View-as (W) to any seat loads that seat's full board (seats 3/4 are now real players).
#
# ⚠ Attacking an off-view opponent from a matchup needs cross-view targeting (the preview tile / Go-back
#   glows to point at it); the engine target-tagging for acting across seats is the deferred piece.
#
...
#   P4 ground SOR_038 / SOR_039(2dmg)   space SOR_086 / SOR_089(3dmg)   base SOR_026 (8 dmg)  leaders Maul+PreVizsla
#
...
#// P3/P4 each run two same-side (Villainy) leaders: P3 = Cad Bane + Aphra, P4 = Maul + Pre Vizsla.
