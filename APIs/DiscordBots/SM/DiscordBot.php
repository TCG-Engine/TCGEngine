...
if (isset($interaction['type']) && $interaction['type'] === 2) {
    ...
    if ($command === "ping") {
		...
		if ($result && $result->num_rows > 0) {
			...
			$responseText = "\nYour Soul Masters DB ID is: " . $userRecord['usersUid'];
		} else {
			$responseText = "\nNo linked Soul Masters DB user found.";
		}
		$query->close();
		$conn->close();
		$response = [
		...
		*/
		...
		if(count($matches) == 1) {
			$uuid = $matches[0];
			$responseText = "Fetching details for: " . CardName($uuid);
			$imageUrl = "https://soulmastersdb.net/TCGEngine/SoulMastersDB/WebpImages/" . $uuid . ".webp";
			$response = [
				'type' => 4, // Respond with a message
				'data' => [
					'content' => $responseText,
			...
			foreach ($matches as $match) {
				$cardName = CardName($match);
				$display = $cardName != null ? $cardName . " (" . $match . ")" : "";
				$button = [
					'type' => 2, // Button type
					'label' => $display, // Display card title
					'style' => 1, // Primary button (blue)
					'custom_id' => 'select_card_' . $match // Unique ID
				];
...
function FindCard($cardSearch, &$response=null) {
	$cardSearch = trim($cardSearch);
	$cardName = CardName($cardSearch);
	if($cardName != null) {
		return [ $cardSearch ];
	}
	else {
		$cardName = strtolower($cardSearch);
		global $NameData;
		$matches = [];
		foreach ($NameData as $uuid => $title) {
