...
UndoStackClear(); UndoCursorSet(-1);
...
$check(UndoStackCount() === 5 && UndoCursor() === 4, 'stack has 5 entries, cursor=4 (got ' . UndoStackCount() . '/' . UndoCursor() . ')');
...
// Undo Phase: restore ord3 (hand=4, the action-phase start), pop -> cursor=2 (the RES boundary now on top).
ob_start(); SWUDoUndo(1, 'phase'); ob_end_clean();
$check($handCount() === 4, 'after Undo Phase: hand == 4 (action-phase start) — got ' . $handCount());
$check(UndoCursor() === 2, 'after Undo Phase: cursor == 2 (RES boundary) — got ' . UndoCursor());
...
$check(UndoCursor() === 1, 'after Undo: cursor == 1 — got ' . UndoCursor());
...
$check(UndoCursor() === -1, 'stack fully unwound: cursor == -1 — got ' . UndoCursor());
