...
| `aspect-filter-hidden-zones.mjs` | The Cards pane's `InAspectFilter` survives a **hidden** deck view. A deck URL with no `playerID` is a spectator, so `GetNextTurn.php` serves the literal `"CardBack"` sentinel for every zone and `Cardaspect()` answers `null` — which used to throw `Cardaspect(...) is null` and render an empty pane. Asserts the hidden view neither throws nor filters, **and** that the owner view still hides off-aspect cards: a guard that just returned "show everything" also stops the error and silently deletes the feature. Reads `DECK` (default `104`) and `ENGINES`. |
| `image-modal-copy-link.mjs` | The deck-image modal's **Copy Link** copies the SAME friendly link as the deck list's Copy Link (pasted back from both and compared; `/deck/<code>` when the deck has one), inside the click turn, with one "Link copied!" flash that is actually drawn **on top of** the modal (it used to sit under the 5000-z overlay). Desktop AND mobile layouts — desktop drives the inline row icons, mobile the ⋮ dropdown — plus a phone-width no-overflow check on the modal's buttons. |
