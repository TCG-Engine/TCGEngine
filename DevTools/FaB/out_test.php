...
$failures=[];
$snapshot=json_decode(file_get_contents(__DIR__.'/out_abilities.json'),true);
set_error_handler(function($severity,$message,$file,$line){throw new ErrorException($message,0,$severity,$file,$line);});
$catalog=json_decode(file_get_contents(__DIR__.'/out_catalog.json'),true);
$check(count($snapshot)===236&&array_column($catalog,'id')===array_column($snapshot,'cardId'),'OUT snapshot does not cover the set.');
$macroChecks=0;
foreach([2,4] as $seatCount)foreach($snapshot as $entry)foreach($entry['abilities'] as $ability){
    ...
    AddArsenal($actor,CardID:'ridge_rider_shot_red'); FaBWTRCreateArena($actor,'ash'); FaBWTRCreateArena($actor,'ash'); FaBWTRCreateArena(1,'yendurai');
...
if($failures){fwrite(STDERR,implode(PHP_EOL,$failures).PHP_EOL);exit(1);}echo "OUT continuations passed; $macroChecks authored continuations exercised in duels and UPF.\n";
