...
$failures=[];
$snapshot=json_decode(file_get_contents(__DIR__.'/dyn_abilities.json'),true);
set_error_handler(function($severity,$message,$file,$line){throw new ErrorException($message,0,$severity,$file,$line);});
$catalog=json_decode(file_get_contents(__DIR__.'/dyn_catalog.json'),true);
$check(count($snapshot)===247&&array_column($catalog,'id')===array_column($snapshot,'cardId'),'DYN snapshot does not cover the set.');
$macroChecks=0;
...
if($failures){fwrite(STDERR,implode(PHP_EOL,$failures).PHP_EOL);exit(1);}echo "DYN continuations passed; $macroChecks authored continuations exercised in duels and UPF.\n";
