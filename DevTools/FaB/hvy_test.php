...
$failures=[];
$snapshot=json_decode(file_get_contents(__DIR__.'/hvy_abilities.json'),true);
set_error_handler(function($severity,$message,$file,$line){throw new ErrorException($message,0,$severity,$file,$line);});
$catalog=json_decode(file_get_contents(__DIR__.'/hvy_catalog.json'),true);
$check(count($snapshot)===255&&array_column($catalog,'id')===array_column($snapshot,'cardId'),'HVY snapshot does not cover the set.');
$macroChecks=0;
foreach([2,4] as $seatCount)foreach($snapshot as $entry)foreach($entry['abilities'] as $ability){
    $arcReset($seatCount); foreach(FaBSeatOrder() as $seat){$soul=&GetSoul($seat);$soul=[];}$actor=$seatCount;$inventory=&GetInventory($actor);$inventory=[];AddInventory($actor,CardID:'spiders_bite',Owner:$actor,Controller:$actor);SetTurnPlayer($actor);SetPriorityPlayer($actor);AddResources($actor,30);
...
if($failures){fwrite(STDERR,implode(PHP_EOL,$failures).PHP_EOL);exit(1);}echo "HVY continuations passed; $macroChecks authored continuations exercised in duels and UPF.\n";
