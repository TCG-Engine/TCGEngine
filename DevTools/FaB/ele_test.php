...
$failures=[];
$snapshot=json_decode(file_get_contents(__DIR__.'/ele_abilities.json'),true);
set_error_handler(function($severity,$message,$file,$line){throw new ErrorException($message,0,$severity,$file,$line);});
$catalog=json_decode(file_get_contents(__DIR__.'/ele_catalog.json'),true);
$check(count($snapshot)===238&&array_column($catalog,'id')===array_column($snapshot,'cardId'),'ELE snapshot does not cover the set.');
$macroChecks=0;
foreach([2,4] as $seatCount)foreach($snapshot as $entry)foreach($entry['abilities'] as $ability){
    ...
    foreach(['autumns_touch_blue','winters_grasp_blue','heavens_claws_blue','pulse_of_isenloft_blue','weave_earth_red','glacial_footsteps_blue'] as $id){AddHand($actor,CardID:$id);AddDeck($actor,CardID:$id);AddGraveyard($actor,CardID:$id);AddSoul($actor,CardID:$id);AddBanish($actor,CardID:$id);}
...
if($failures){fwrite(STDERR,implode(PHP_EOL,$failures).PHP_EOL);exit(1);}echo "ELE continuations passed; $macroChecks authored continuations exercised in duels and UPF.\n";
