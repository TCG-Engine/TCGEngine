...
$failures=[];
$snapshot=json_decode(file_get_contents(__DIR__.'/dtd_abilities.json'),true);
set_error_handler(function($severity,$message,$file,$line){throw new ErrorException($message,0,$severity,$file,$line);});
$catalog=json_decode(file_get_contents(__DIR__.'/dtd_catalog.json'),true);
$check(count($snapshot)===245&&array_column($catalog,'id')===array_column($snapshot,'cardId'),'DTD snapshot does not cover the set.');
$macroChecks=0;
foreach([2,4] as $seatCount)foreach($snapshot as $entry)foreach($entry['abilities'] as $ability){
    ...
    AddArsenal($actor,CardID:'ridge_rider_shot_red'); FaBWTRCreateArena($actor,'figment_of_protection_yellow'); FaBWTRCreateArena($actor,'ash'); FaBWTRCreateArena(1,'yendurai');
...
if($failures){fwrite(STDERR,implode(PHP_EOL,$failures).PHP_EOL);exit(1);}echo "DTD continuations passed; $macroChecks authored continuations exercised in duels and UPF.\n";
