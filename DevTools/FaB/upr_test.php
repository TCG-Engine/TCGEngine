...
$failures=[];
$snapshot=json_decode(file_get_contents(__DIR__.'/upr_abilities.json'),true);
set_error_handler(function($severity,$message,$file,$line){throw new ErrorException($message,0,$severity,$file,$line);});
$catalog=json_decode(file_get_contents(__DIR__.'/upr_catalog.json'),true);
$check(count($snapshot)===238&&array_column($catalog,'id')===array_column($snapshot,'cardId'),'UPR snapshot does not cover the set.');
$macroChecks=0;
...
if($failures){fwrite(STDERR,implode(PHP_EOL,$failures).PHP_EOL);exit(1);}echo "UPR continuations passed; $macroChecks authored continuations exercised in duels and UPF.\n";
