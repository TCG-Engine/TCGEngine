...
$failures=[];
$snapshot=json_decode(file_get_contents(__DIR__.'/mst_abilities.json'),true);
set_error_handler(function($severity,$message,$file,$line){throw new ErrorException($message,0,$severity,$file,$line);});
$catalog=json_decode(file_get_contents(__DIR__.'/mst_catalog.json'),true);
$check(count($snapshot)===240&&array_column($catalog,'id')===array_column($snapshot,'cardId'),'MST snapshot does not cover the set.');
$macroChecks=0;
foreach([2,4] as $seatCount)foreach($snapshot as $entry)foreach($entry['abilities'] as $ability){
    ...
    FaBRunSourceMacro($macro,$actor,$id,['mzID'=>FaBFindUID($uid)['mzID'],'hvyAttackUID'=>$uid,'hvyTokens'=>['gold'],'hvyMandatory'=>false,'evoVictim'=>1,'eleTarget'=>1,'eleRepeats'=>1,'amount'=>2,'attacker'=>$actor,'defender'=>1]);
...
if($failures){fwrite(STDERR,implode(PHP_EOL,$failures).PHP_EOL);exit(1);}echo "MST continuations passed; $macroChecks authored continuations exercised in duels and UPF.\n";
