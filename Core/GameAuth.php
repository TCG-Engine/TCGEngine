...
function SimGameValidateSpectatorAuth($rootName, $gameName, $authKey = '')
{
  ...
  if (!SimGameIsPrivateGame($rootName, $gameName)) {
    // Public game: open to every viewer, account or not. SWUSim required a login here until 2026-09-21 (owner: guests
    // may spectate now that they may play); guests still cannot chat (SubmitChat.php).
    return true;
  }
