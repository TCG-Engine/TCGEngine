...
> - **triage every card against the THREE LIVE FORMATS from its printed wording** — Premier/Eternal (2P),
>   Twin Suns (3–4P, every player for themselves) and Team Suns (4P, 2v2) — and write only the sections
>   the wording actually implicates. A player reference ("an opponent"/"a player" = a PROMPT · "each
>   opponent/player" = a LOOP over live seats · "that player"/"its controller" = already DETERMINED, no
>   prompt) earns a **Twin Suns section that cannot pass at two seats**; the word **"friendly"/"enemy"**
>   earns a **Team Suns section** (`WithTeams: true`); **"you control"** earns neither and stays self-only.
>   Never hand-roll `[1,2]` or `OtherPlayer()`. Note the verdict in the `modes=` field of the COVERAGE
>   ledger, including when it is "2P only" — it is a guideline for thinking the card through once, not a
>   gate to satisfy.
>   ⚠ Do NOT write all three for every card: ~2/3 of cards touch neither axis and the three formats are
>   the same code path for them, so the extra sections can never fail and only dilute the signal. But
>   ⚠ the cards that DO name a player and lack a far-seat section were ~50% genuinely broken when swept,
>   so that axis is where the time pays;
...
| Skipping the FORMAT TRIAGE, or padding it | Three formats are live (2P · Twin Suns 3–4P · Team Suns 4P 2v2). Decide from the PRINTED WORDING which ones a card implicates and write only those: a player reference → a Twin Suns section that cannot pass at two seats; "friendly"/"enemy" → a Team Suns section (`WithTeams: true`); "you control" → neither. Record it as `modes=` in the COVERAGE ledger, "2P only" included. ⚠ Both failure directions are real: ~2/3 of cards touch neither axis, so blanket 3× sections can never fail and just train readers to skim — while cards that DO name a player and had no far-seat section were **~50% genuinely broken** when swept (Vonreg + all 10 pilot leaders, Whistling Birds, TIE Bomber, Fallen Lightsaber, and 14 more). ⚠ Twin Suns and Team Suns share one code path for any card that does not say "friendly", so a Team Suns section there is a duplicate. |
| Measuring the section FLOOR with `Epic Action` counted as a clause, or one combined floor for a two-sided leader | Both INVENT a backlog. Epic deploy is generic engine behaviour needing no per-leader tests, so it must not raise the floor; and a leader's floor is **per SIDE, summed** (1 front clause + 2 deployed = 4+7=11, not "3 clauses → 10"). Measured wrong, SOR read 6 files / 10 sections short when the honest answer was 2 files / 3 sections. |
| Treating the `COVERAGE:` ledger as paperwork | Writing it is what FINDS the backlog. The axis scan only asks whether a file mentions an assertion family anywhere; the ledger asks, per clause, whether *that* cell is walked and forces every N/A to be justified structurally. Retrofitting SOR's last 25 files surfaced **19 open cells** on a set already reading 0% missing on five axes. |
| Writing a far-seat section whose ACTOR is seat 3 or 4 || Writing a far-seat section whose ACTOR is seat 3 or 4 | There is no `WithP3Leader`, and `CommonSetup` dresses seats 1-2 only — so `P3>DeployLeader` (or anything needing a far seat's leader) is a SILENT NO-OP and the section passes because nothing happened. Put the ACTOR on seat 1 or 2 and the far-seat ROLE on 3/4; seats 3/4 are for boards (arenas, bases, hands, decks), not leaders. Caught only by a green mutation. |
| Testing a RANDOM-target card on a board where the wrong reading is a coin flip | With no decision to assert, the pool is only testable behaviourally — and a 2-member wrong pool makes the section pass ~half the time, often under the very mutation written to catch it. Build boards where BOTH readings are deterministic: the wrong pool EMPTY vs exactly one member, or two IDENTICAL bodies so whichever is picked dies. |
| Writing a `COVERAGE:` N/A that is SITUATIONAL rather than STRUCTURAL | An N/A is a claim, and claims rot: **19 of SOR's were wrong or stale** when verified rather than read. "N/A — the buff is self-referential, nothing is chosen" survives a rewrite; "N/A — only one legal target" is falsified by the next fixture change and nothing tells you. And when a later batch touches a card's handler, re-read that card's ledger **in the same edit** — it is two lines above the section you are already in. |
| Asserting survival with `HP:` alone | `P{n}GROUNDARENAUNIT:i:HP:` is CURRENT MAX HP — printed + modifiers, **damage NOT subtracted**. A unit on 5 HP with 4 damage still reads `HP:5`. Survival needs `HP:` **and** `DAMAGE:`, or `ARENACOUNT` for the death. |
...
| Deriving the card list ONCE, at the start of a preview run | **A preview set can grow WHILE you are working.** `AppCore/SWU/CardMocks.php` was rewritten THREE times during the 2026-08-26 HMW run (103 → 108 → 109 CardIDs), so the list derived at the start was stale twice over — and the second growth landed *after* a diff had just reported the set complete. **Re-run the `### Already Done` vs `CardMocks.php` diff at the END of the run as well as the start**, and treat any "card-complete" claim as true only as of that last diff. Say so when reporting it. |
...
| Shipping a clause that ASSUMES its trigger landed, or a helper that swallows an unconditional "Then" | The TS26 port's biggest family — **three bugs in one set**. Decide from the WORDING whether a gated clause measures the outcome. ⚠ JUDGE RULING 2026-09-14: **"deal N damage … If you do" is satisfied even when the damage is PREVENTED** (gate on the choice; prevented damage fires no damage triggers). "**for each damage dealt this way**", "ready/exhaust X. If you do" (CR 1.e) and "if it is defeated" DO measure (sample before/after). Base-damage prevention (JTL_074 Close the Shield Gate), no-heal locks (SOR_160, TWI_132), can't-ready (SHD_193), Shields and can't-be-defeated are the fixtures that make the difference observable. Mirror image: a clause joined by "**Then**" is UNCONDITIONAL, so when the first half is delegated to a shared helper, check what that helper does with EMPTY input — `_topDeckSearchBegin` no-ops on an empty deck and took Captain Vaughn's second clause down with it. See the `swusim-implement-card` "More recurring bug shapes (TS26 …)" block. |
| Deriving a rules split from CARD TEXT instead of an authoritative per-card list | **The most expensive near-miss of the TS26 port.** CR 7.6.16.c makes "When Attack Ends" fire by DEFAULT when its own unit dies, with survival as a per-card OPT-IN. Classifying by phrasing ("Attack Ends" vs bare "completes an attack" vs "(and survives)") misclassified **11 cards** that read as bare "completes" but genuinely require survival — shipping it would have BROKEN 11 working cards to fix 5, and the same heuristic produced a blast-radius estimate off by ~6×. When a behaviour splits cards into two groups, build the split as an explicit CardID list and decide each new card's membership deliberately; card text is written for humans and omits riders the rules take as read. **Count affected cards from the list, never from a grep.** |
