...
global $onAttackAbilities, $onDefenseAbilities, $onAttackEndAbilities;
...
$customDQHandlers["SOR_022"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["SOR_019"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["SOR_005"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-') {
...
$customDQHandlers["SOR_010"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["SHD_012"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') {
        SWUAfterAction(intval($player));
        return;
    }
...
$onAttackAbilities["SOR_244:0"] = function($player) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_244", 0);
};
$customDQHandlers["SOR_244"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["JTL_001"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["JTL_003"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
        SWUAfterAction(intval($player));
        return;
    }
...
// end-of-action.
$customDQHandlers["JTL_005"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
        SWUAfterAction(intval($player));
        return;
    }
...
$customDQHandlers["JTL_008"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
...
$onAttackAbilities["JTL_010:0"] = function($player, $mzID) {
    ...
    if (empty($targets)) return;
    SWUQueueMayChooseTarget(intval($player), $targets,
        "You_may_deal_1_damage_to_a_unit", "Deal_1_damage_to_a_unit", "JTL_010");
};

// Deploy continuation: deal 1 to the chosen unit ($lastDecision), then "if you do" deal 1 to a base.
$customDQHandlers["JTL_010"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["JTL_011"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
        SWUAfterAction(intval($player));
        return;
    }
...
$customDQHandlers["JTL_014"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["JTL_015"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
        SWUAfterAction(intval($player));
        return;
    }
...
$customDQHandlers["JTL_016"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$onAttackAbilities["JTL_016:0"] = function($player, $mzID) {
    ...
    if (empty($targets)) return;
    SWUQueueMayChooseTarget(intval($player), $targets,
        "You_may_exhaust_a_unit", "Exhaust_a_unit_(its_controller_creates_an_X-Wing)", "JTL_016");
};
...
$customDQHandlers["JTL_018"] = function($player, $parts, $lastDecision) {
    if ($lastDecision && $lastDecision !== '-' && $lastDecision !== '' && $lastDecision !== 'PASS') {
        $o = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_017"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
        SWUAfterAction(intval($player));
        return;
    }
...
$whenPlayedAbilities["JTL_051:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, 'CUSTOM', "JTL_051|" . $mzID, 1);
};
$customDQHandlers["JTL_051"] = function($player, $parts, $lastDecision) {
...
$whenPlayedAbilities["JTL_140:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "JTL_140", 1);
};
$customDQHandlers["JTL_140"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["JTL_144"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    $o = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_253"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["JTL_194"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    OnExhaustCard(intval($player), $lastDecision);
...
$customDQHandlers["JTL_042"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    $o = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_122"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
...
$whenDefeatedAbilities["JTL_063:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, 'CUSTOM', 'JTL_063', 1);
};
$customDQHandlers["JTL_063"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES') return;
    DoDrawCard(intval($player), 1);
};

// ── JTL_089 The Invisible Hand — When Played: search the top 8 cards of your deck for a Droid unit,
// reveal it, and draw it. "If it costs 2 or less, you may play it for free." Uses a JTL_089-specific
// finalize so the drawn Droid can route into the optional free-play rider. (The complete-attack copy
// is still deferred.) ──────────────────────────────────────────────────────────────────────────────
$whenPlayedAbilities["JTL_089:0"] = function($player, $mzID) {
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["JTL_089"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
$whenPlayedAbilities["JTL_119:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, 'CUSTOM', 'JTL_119', 1);
};
$customDQHandlers["JTL_119"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES') return;
    global $playerID;
    $playerID = intval($player);
...
$whenPlayedAbilities["JTL_164:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($me, 'CUSTOM', 'JTL_119', 1);
};
...
$jtl154_choose = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision(intval($player), "CUSTOM", "JTL_154|" . intval($player), 1);
};
$whenPlayedAbilities["JTL_154:0"]   = $jtl154_choose;
$whenDefeatedAbilities["JTL_154:0"] = $jtl154_choose;
$customDQHandlers["JTL_154"] = function($player, $parts, $lastDecision) {
...
$whenPlayedAbilities["JTL_201:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, 'CUSTOM', "JTL_201|" . intval($player), 1);
};
$customDQHandlers["JTL_201"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $caster = intval($parts[0]);
    $playerID = $caster;
...
$whenPlayedAbilities["JTL_047:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, 'CUSTOM', "JTL_047|{$uid}", 1);
};
$customDQHandlers["JTL_047"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["JTL_192"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
    $mz  = $parts[0] ?? '';
...
$onAttackAbilities["JTL_227:0"] = function($player, $mzID) {
    ...
    if (empty($targets)) return;
    SWUQueueMayChooseTarget(intval($player), $targets,
        "Exhaust_an_enemy_unit_(deal_indirect_equal_to_its_power)", "Choose_an_enemy_non-leader_unit", "JTL_227");
};
$customDQHandlers["JTL_227"] = function($player, $parts, $lastDecision) {
...
$whenPlayedAbilities["JTL_097:0"] = function($player, $mzID) {
    ...
    SWUQueueMayChooseTarget(intval($player), $pilots,
        "Attack_with_a_Pilot_unit_(+1/+0,_Restore_1)", "Choose_a_Pilot_unit_to_attack_with", "JTL_097");
};
$customDQHandlers["JTL_097"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_123"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_124"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_202"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES') return;
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["JTL_228"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_231"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_261"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_156"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_177"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_193"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_235"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_244"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '' || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    foreach (explode("&", $lastDecision) as $mz) {
...
$jtl219 = function($player, $mzID) {
    ...
    SWUQueueChooseTarget(intval($player), $friendly, "Deal_1_to_a_friendly_unit_(then_ready_a_resource)", "JTL_219");
};
$whenPlayedAbilities["JTL_219:0"] = $jtl219;
$onAttackAbilities["JTL_219:0"]   = $jtl219;
$customDQHandlers["JTL_219"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
$onAttackAbilities["JTL_152:0"] = function($player, $mzID) {
    ...
    if ($power <= 0) return;
    SWUDealIndirectDamage(intval($player), $power, OtherPlayer(intval($player)), "JTL_152");
};
$customDQHandlers["JTL_152"] = function($player, $parts, $lastDecision) {
...
$whenPlayedAbilities["JTL_218:0"] = function($player, $mzID) {
    ...
    SWUDealIndirectToChosenPlayer(intval($player), 3, "JTL_218~{$uid}");
};
$customDQHandlers["JTL_218"] = function($player, $parts, $lastDecision) {
...
$whenPlayedAbilities["JTL_222:0"] = function($player, $mzID) {
    global $playerID; $playerID = intval($player);
    SWUDealIndirectToChosenPlayer(intval($player), 3, "JTL_222");
};
// JTL_009 Boba Fett — resolve the "exhaust this leader → 1 indirect" non-combat-damage reaction.
$customDQHandlers["JTL_009"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["JTL_222"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["JTL_129"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_180"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_043"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
    if ($obj === null || !empty($obj->removed)) return;
    $uid   = intval($obj->UniqueID ?? 0);
    SWUTakeControlOfUnit(intval($player), $lastDecision);   // unit moves into the caster's arena
    $mz = SWUFindMzByUID($uid);
    if ($mz !== null) SWUDefeatUnit(intval($player), $mz);  // then defeat it (to its owner's discard)
};
...
$customDQHandlers["JTL_205"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '' || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $o = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_232"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    $o = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_233"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    $picks = [];
...
$customDQHandlers["JTL_195"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["JTL_206"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
...
// arena. ─────────────────────────────────────────────────────────────────────────────────────────────
$customDQHandlers["JTL_178"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
...
// ── JTL_070 U-Wing Lander — When Played: Give 3 Experience tokens to this unit. (The complete-attack
// "move an upgrade to another Vehicle" rider is deferred to the Phase 16/18 upgrade-move work.) ───────
...
$whenPlayedAbilities["JTL_072:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "JTL_072", 1);
};
$customDQHandlers["JTL_072"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    $picks = array_slice(array_filter(explode("&", $lastDecision), fn($m) => $m !== '' && $m !== '-'), 0, 2);
...
$onAttackAbilities["JTL_250:0"] = function($player, $mzID) {
    ...
    if (_SWUControlsUnitWithAspect($p, 'Cunning')) {
        ...
        DecisionQueueController::AddDecision($p, "CUSTOM", "JTL_250", $block);
        $block++;
    }
};

$customDQHandlers["JTL_250"] = function($player, $parts, $lastDecision) {
    $p = intval($player);
...
$whenPlayedAbilities["JTL_039:0"] = function($player, $mzID) {
    ...
    SWUQueueMayChooseTarget(intval($player), $targets,
        ...
        "Use_a_When_Defeated_ability_on_another_friendly_unit", "JTL_039");
};

$customDQHandlers["JTL_039"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    $obj = GetZoneObject($lastDecision);
    if ($obj === null || !empty($obj->removed)) return;
...
$customDQHandlers["JTL_126"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
...
$whenPlayedAbilities["JTL_038:0"] = function($player, $mzID) {
    ...
    if (empty($targets)) return;
    SWUQueueMayChooseTarget(intval($player), $targets,
        "Attach_a_friendly_Pilot_to_this", "Attach_a_friendly_Pilot_to_this", "JTL_038|" . $mzID);
};

$customDQHandlers["JTL_038"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    $corvusMz = $parts[0] ?? '';
...
$customDQHandlers["JTL_175"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
// (if it survived). ──────────────────────────────────────────────────────────────────────────────────
$customDQHandlers["JTL_230"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    $o = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_091"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
...
$whenPlayedAbilities["JTL_041:0"] = $whenDefeatedAbilities["JTL_041:0"] = function($player, $mzID) {
    ...
    if (empty($targets)) return;
    SWUQueueMayChooseTarget(intval($player), $targets,
        "You_may_defeat_an_enemy_unit_(name-hunt_their_deck_and_hand)", "Defeat_an_enemy_unit", "JTL_041");
};
$customDQHandlers["JTL_041"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    $o = GetZoneObject($lastDecision);
...
$customDQHandlers["JTL_055"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    SWUDefeatUnit(intval($player), $lastDecision);
...
// then deal 3 to each. ───────────────────────────────────────────────────────────────────────────────
$customDQHandlers["JTL_173"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["JTL_176"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    $o = GetZoneObject($lastDecision);
...
$whenPlayedAbilities["JTL_170:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "JTL_170", 1);
};
$customDQHandlers["JTL_170"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS' || $lastDecision === '') return;
    global $playerID;
    $playerID = intval($player);
    $uids = [];
...
$onAttackAbilities["JTL_142:0"] = function($player, $mzID) {
    ...
    if (empty($units)) return;
    SWUQueueMayChooseTarget(intval($player), $units, "Deal_1_damage_to_a_unit", "Choose_a_unit", "JTL_142");
};
$customDQHandlers["JTL_142"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
    ...
    $targets[] = 'theirBase-0';
    $targets[] = 'myBase-0';
    SWUQueueMayChooseTarget(intval($player), $targets, "Deal_1_damage_to_a_unit_or_base", "Choose_a_target", "JTL_1422");
};
$customDQHandlers["JTL_1422"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$whenPlayedAbilities["JTL_210:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "JTL_210", 1);
};
$customDQHandlers["JTL_210"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '' || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    foreach (explode("&", $lastDecision) as $mz) {
...
$whenPlayedAsUpgradeAbilities["JTL_203:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, 'CUSTOM', "JTL_203|{$uid}", 1);
};
$customDQHandlers["JTL_203"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES') return;
    global $playerID;
    $playerID = intval($player);
    $mz = SWUFindMzByUID(intval($parts[0] ?? 0));
...
$whenPlayedAsUpgradeAbilities["JTL_145:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, 'CUSTOM', 'JTL_145', 1);
};
$customDQHandlers["JTL_145"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES') return;
    global $playerID;
    $playerID = intval($player);
...
$onAttackAbilities["SOR_142:0"] = function($player, $mzID) {
    ...
    SWUQueueMayChooseTarget(intval($player), $targets,
        "You_may_deal_1_to_the_defender_or_a_base", "Deal_1_damage_to_the_defender_or_a_base", "SOR_142");
};

$customDQHandlers["SOR_142"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    if (strpos($lastDecision, 'Base') !== false) {
...
$whenPlayedAbilities["SOR_097:0"] = function($player, $mzID) {
    ...
    SWUQueueMayChooseTarget(intval($player), $targets,
        ...
        "Choose_a_unit_to_damage", "SOR_097");
};

$customDQHandlers["SOR_097"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$onAttackAbilities["SOR_158:0"] = function($player, $mzID) {
    ...
    SWUQueueChooseTarget(intval($player), $targets,
        "Deal_2_damage_to_a_ground_unit_or_a_base", "SOR_158");
};

$customDQHandlers["SOR_158"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    if (strpos($lastDecision, 'Base') !== false) {
...
$onAttackAbilities["SOR_156:0"] = function($player, $mzID) {
    ...
    if (empty($targets)) return;
    SWUQueueChooseTarget(intval($player), $targets,
        "Grant_Raid_2_to_another_friendly_Aggression_unit", "SOR_156");
};

$customDQHandlers["SOR_156"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$whenPlayedAbilities["SOR_102:0"] = function($player, $mzID) {
    ...
    if (empty($targets)) return;
    SWUQueueChooseTarget(intval($player), $targets, "Play_a_Heroism_unit_from_your_discard_(costs_3_less)", "SOR_102");
};

$customDQHandlers["SOR_102"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["SOR_075"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["SOR_055"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $mz = $lastDecision;
...
$customDQHandlers["SOR_168"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $savedPID = $playerID;
    $playerID = intval($player);
    $mz = $lastDecision ?? '';
...
$customDQHandlers["SOR_150"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $savedPID = $playerID;
    $playerID = intval($player);
    $mz = $lastDecision ?? '';
...
$customDQHandlers["SOR_106"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["JTL_100"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["JTL_013"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
    $hostMz = $lastDecision ?? '';
    if ($hostMz === '' || $hostMz === '-') {
...
$whenPlayedAbilities["SOR_215:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_215|{$player}|{$mzID}", 1);
};

// SOR_215: player answered the "Attack with attached unit?" YESNO.
// $parts[0] = player, $parts[1] = host unit mzID.
global $customDQHandlers;
$customDQHandlers["SOR_215"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $savedPID = $playerID;
    $playerID = intval($player);
    $unitMzID = $parts[1] ?? '';
...
$whenPlayedAbilities["SOR_101:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_101", 1);
};

$customDQHandlers["SOR_101"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '' || $lastDecision === '-' || $lastDecision === 'PASS') return;
...
$onAttackAbilities["SOR_047:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision(intval($player), 'CUSTOM', 'SOR_047', 1);
};

$customDQHandlers["SOR_047"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES') return;
    global $playerID;
    $playerID = intval($player);
...
$whenDefeatedAbilities["SOR_204:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision(intval($player), 'CUSTOM', 'SOR_204', 1);
};

$customDQHandlers["SOR_204"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES') return;
    global $playerID;
    $playerID = intval($player);
    $milled = SWUMillTopCard(intval($player));
...
$whenPlayedAbilities["SOR_183:0"] = function($player, $mzID) {
    ...
    SWUQueueMayChooseTarget(intval($player), $events,
        'Return_an_event_from_a_discard_pile_to_its_owner\'s_hand', 'Choose_an_event_to_return', 'SOR_183');
};

$customDQHandlers["SOR_183"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '' || $lastDecision === '-' || $lastDecision === 'PASS') return;
...
$whenPlayedAbilities["SOR_197:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision(intval($player), 'CUSTOM', 'SOR_197', 1);
};

$customDQHandlers["SOR_197"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '' || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$whenPlayedAbilities["SOR_080:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_080", 1);
};

$customDQHandlers["SOR_080"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === "" || $lastDecision === "-" || $lastDecision === "PASS") return;
    foreach (explode("&", $lastDecision) as $mz) {
        if ($mz === "" || $mz === "-" || $mz === "PASS") continue;
        DoGiveExperienceToken(intval($player), $mz);
...
$customDQHandlers["SOR_235"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["SOR_138"] = function($player, $parts, $lastDecision) {
    global $playerID; $playerID = intval($player);
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    $o = GetZoneObject($lastDecision);
    if ($o === null || !empty($o->removed)) return;
...
$customDQHandlers["SOR_167"] = function($player, $parts, $lastDecision) {
    global $playerID;
...
$customDQHandlers["SOR_233"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $caster = intval($player);
    $playerID = $caster;
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
...
$customDQHandlers["SOR_187"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $caster = intval($player);
    $playerID = $caster;
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
...
$whenDefeatedAbilities["SOR_145:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision(intval($player), "CUSTOM", "SOR_145|{$opp}", 1);
};
$customDQHandlers["SOR_145"] = function($player, $parts, $lastDecision) {
    global $playerID;
...
$customDQHandlers["SOR_174"] = function($player, $parts, $lastDecision) {
    global $playerID;
...
$customDQHandlers["SOR_223"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    $o = GetZoneObject($lastDecision);
    ...
    if ($cardID === 'SOR_107') {                          // Command
        switch ($label) {
            ...
            case 'PowerStrike': {                         // a friendly unit deals its power to a non-unique enemy
                ...
                SWUQueueChooseTarget($player, array_values($friendly), "Choose_the_friendly_unit", "SOR_107");
                return;
            }
    ...
    if ($cardID === 'SOR_203') {                          // Cunning
        switch ($label) {
            ...
            case 'Exhaust': {                             // exhaust up to 2 units
                ...
                DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_203", 1);
                return;
            }
...
$customDQHandlers["SOR_107"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
...
$customDQHandlers["SOR_203"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    foreach (explode("&", $lastDecision) as $mz) {
...
$customDQHandlers["SOR_139"] = function($player, $parts, $lastDecision) {
    global $playerID; $playerID = intval($player);
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    $o = GetZoneObject($lastDecision);
    if ($o === null || !empty($o->removed)) return;
...
$customDQHandlers["SOR_245"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === "" || $lastDecision === "-" || $lastDecision === "PASS") return;
    foreach (explode("&", $lastDecision) as $mz) {
        if ($mz === "" || $mz === "-" || $mz === "PASS") continue;
        DoGiveExperienceToken(intval($player), $mz);
...
$customDQHandlers["SOR_173"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["SOR_127"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["SOR_220"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $savedPID = $playerID;
    $playerID = intval($player);
    $attackerMzID = $lastDecision ?? '';
...
$customDQHandlers["SOR_234"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $imp1Mz = $lastDecision;
...
$customDQHandlers["SOR_252"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '' || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $byOwner = [1 => [], 2 => []];
...
$whenPlayedAbilities["SOR_033:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_033", 1);
};
$customDQHandlers["SOR_033"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["SOR_092"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$whenPlayedAbilities["SOR_052:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_052|{$selfUID}", 1);
};
...
$customDQHandlers["SOR_052"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
$whenPlayedAbilities["SOR_176:0"] = function($player, $mzID) {
    ...
    SWUQueueMayChooseTarget(intval($player), $targets,
        "Reveal_an_event_from_your_hand_to_deal_1_damage?", "Deal_1_damage_to_a_unit", "SOR_176");
};
$customDQHandlers["SOR_176"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["SOR_151"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $friendlyMz = $lastDecision;
...
$unitAbilities["JTL_146"] = function($player, $mzID) {
    ...
    SWUQueueChooseTarget(intval($player), $fighters, "Choose_a_Fighter_to_attack_with_(+2/+0)", "JTL_146");
};
$customDQHandlers["JTL_146"] = function($player, $parts, $lastDecision) {
...
$onAttackAbilities["JTL_186:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, 'YESNO', '-', 1, tooltip: "Draw_a_card?");
    DecisionQueueController::AddDecision($player, 'CUSTOM', 'JTL_186', 1);
};
$customDQHandlers["JTL_186"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES') return;
    DoDrawCard(intval($player), 1);
};
...
$customDQHandlers["SOR_003"] = function($player, $parts, $lastDecision) {
...
$customDQHandlers["SOR_219"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
...
$customDQHandlers["TWI_005"] = function($player, $parts, $lastDecision) {
...
$unitAbilities["SOR_110"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_110", 1);
};
// BeginSWUAttack (combat) owns the after-action — do NOT append SWU_AFTER_ACTION.
$customDQHandlers["SOR_110"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
        SWUAfterAction($player);
        return;
    }
...
$whenPlayedAbilities["SOR_099:0"] = function($player, $mzID) {
    ...
    SWUQueueMayChooseTarget(intval($player),
        ...
        'Return_a_friendly_ground_unit_to_hand_(then_draw)?', 'Choose_a_friendly_ground_unit_to_return', 'SOR_099');
};
$customDQHandlers["SOR_099"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["SOR_221"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
    $arena = ($lastDecision === 'Space') ? 'SpaceArena' : 'GroundArena';
    foreach (array_merge(
...
$onAttackAbilities["SOR_206:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, 'CUSTOM', 'SOR_206', 1);
};
$customDQHandlers["SOR_206"] = function($player, $parts, $lastDecision) {
...
$whenDefeatedAbilities["SOR_147:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, 'CUSTOM', 'SOR_147', 1);
};
$customDQHandlers["SOR_147"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES' && $lastDecision !== '1') return;
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["SOR_171"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["SOR_186"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $o = GetZoneObject($lastDecision);
...
$customDQHandlers["SOR_115"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES') return;
    global $playerID;
    $playerID = intval($player);
    DoDrawCard(intval($player), 1);
...
$customDQHandlers["SOR_013"] = function($player, $parts, $lastDecision) {
    if ($lastDecision !== 'YES') return;
    global $playerID;
    $playerID = intval($player);
    DoDrawCard(intval($player), 1);
...
$whenDefeatedAbilities["SOR_049:0"] = function($player, $mzID) {
    ...
    if (empty($targets)) return;
    SWUQueueChooseTarget(intval($player), $targets, "Give_2_Experience_to_another_friendly_unit", "SOR_049");
};
$customDQHandlers["SOR_049"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$whenPlayedAbilities["SOR_140:0"] = function($player, $mzID) {
    ...
    if (empty($targets)) return;
    SWUQueueChooseTarget(intval($player), $targets, "Choose_a_unit_to_lose_Sentinel", "SOR_140");
};

// Tag with the bare CardID — drives SWUKeywordSuppressed (via $keywordSuppressors)
// and doubles as the Active Effects UI source.
$customDQHandlers["SOR_140"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$onAttackAbilities["SOR_119:0"]   = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_119", 1);
};

// Receives the OPTIONCHOOSE label. Both branches act on the top (front) card just looked at.
$customDQHandlers["SOR_119"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
$onAttackEndAbilities["SOR_192:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_192", 1);
};

$customDQHandlers["SOR_192"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
    if ($lastDecision === 'Discard') {
        SWUMillTopCard(intval($player));
...
$onAttackAbilities["SOR_238:0"]   = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_238", 1);
};
...
$customDQHandlers["SOR_238"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
    $idx = _SWUTopDeckFrontIdx(intval($player));
    if ($idx === -1) return;
...
$customDQHandlers["SOR_246"] = function($player, $parts, $lastDecision) {
...
$whenPlayedAbilities["SOR_062:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_062|{$uid}", 1);
};

$customDQHandlers["SOR_062"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    $uid = intval($parts[0] ?? 0);
    if ($uid === 0) return;
...
$onAttackAbilities["SOR_185:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_185", 1);
};
...
$customDQHandlers["SOR_185"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$whenPlayedAbilities["SOR_189:0"] = function($player, $mzID) {
    ...
    DecisionQueueController::AddDecision($player, "CUSTOM", "SOR_189|{$mzID}", 1);
};

// Receives the OPTIONCHOOSE label. $parts[0] = Leia's own arena mzID (excluded from exhaust targets).
$customDQHandlers["SOR_189"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
    $leiaMzID  = $parts[0] ?? '';
...
$customDQHandlers["SOR_224"] = function($player, $parts, $lastDecision) {
...
//   #1: draw
$customDQHandlers["SOR_006"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === 'PASS') {
        SWUAfterAction(intval($player));
        return;
    }
...
$customDQHandlers["SOR_016#2"] = function($player, $parts, $lastDecision) {
    ...
    DecisionQueueController::AddDecision($player, 'CUSTOM', 'SOR_016|attack', 1);
};
...
$customDQHandlers["SOR_016"] = function($player, $parts, $lastDecision) {
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["SOR_017"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
        SWUAfterAction(intval($player));
        return;
    }
...
$customDQHandlers["SOR_018"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
        SWUAfterAction($player);
        return;
    }
...
$customDQHandlers["SOR_009"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
        SWUAfterAction($player);
        return;
    }
...
$customDQHandlers["SOR_103"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["SOR_012"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') {
        SWUAfterAction($player);
        return;
    }
...
$customDQHandlers["SOR_011"] = function($player, $parts, $lastDecision) {
    if ($lastDecision === null || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
    $obj = GetZoneObject($lastDecision);
...
$customDQHandlers["SOR_193"] = function($player, $parts, $lastDecision) {
    $falconMz = $parts[0] ?? '';
    if ($falconMz === '') return;
    global $playerID;
    $savedPID = $playerID;
...
$customDQHandlers["SHD_131"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
...
$customDQHandlers["TWI_115"] = function($player, $parts, $lastDecision) {
    if (!$lastDecision || $lastDecision === '-' || $lastDecision === '' || $lastDecision === 'PASS') return;
    global $playerID;
    $playerID = intval($player);
