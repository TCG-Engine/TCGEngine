...
# Phase 3 — a third site on the same box: upf.talishar.net → FaBSim

Adds `upf.talishar.net` → FaBSim (db `fabsim`) to the clarent/northbeach box. Unlike phase 2, the
fallback does **not** move: northbeach.gg/HellbreakSim stays the unmatched-`Host` site.

## Target end state

| | |
|---|---|
| `clarent.net`, `www.clarent.net` | db `grandarchivesim` → `httpd-vhost-100-grandarchivesim.conf` (unchanged) |
| `northbeach.gg`, `www.northbeach.gg` | db `hellbreaksim` → `httpd-vhost-000-hellbreaksim.conf` (unchanged; still the fallback) |
| `upf.talishar.net` | db `fabsim` → **`httpd-vhost-100-fabsim.conf`** |
| TLS | XAMPP's self-signed cert again. The talishar.net zone is on Cloudflare **Full** (confirmed 2026-09-16), which accepts it. **Never switch talishar.net to Full (strict)** without an Origin CA cert for `upf.talishar.net` first — that zone also serves Talishar itself. |

Repo prerequisites (all tracked, must be on the box before step 3.3):
`Database/SiteRegistry.php` has `'FaBSim' => ['db' => 'fabsim', 'site' => true]` (plus FaBDeck,
`site => false`), and `newhost/provision-vhost.sh` supports `--no-catch-all`.

## Why `--no-catch-all`

Without a flag the script writes the deny-403 `httpd-vhost-000-default.conf`, which sorts before
`000-hellbreaksim.conf` and **steals the fallback** (see "Why a rename, not a re-run" above).
`--default-site` would write `000-fabsim.conf`, which also sorts first and makes FaBSim the
fallback. `--no-catch-all` writes a plain `100-fabsim.conf` and nothing else, and refuses to run
unless a `000-<app>.conf` fallback already exists.

## Step 3.0 — DNS (Cloudflare, talishar.net zone)

Add `A  upf  <this box's public IP>`, **proxied** (orange cloud). Check afterwards:
```bash
dig +short upf.talishar.net A      # expect Cloudflare IPs, not the origin
```
As in phase 2, nothing below needs DNS — every routing check uses `curl -H 'Host: …' 127.0.0.1`.

## Step 3.1 — Discovery (read-only) — **done 2026-09-16**

Box state as captured (re-run and compare if time has passed):
```bash
sudo /opt/lampp/bin/httpd -DSSL -DPHP -S          # NOT bare -S: that hides every :443 vhost
ls -l /opt/lampp/etc/extra/httpd-vhost-*.conf
grep -nE "httpd-vhost|httpd-ssl.conf|^Listen" /opt/lampp/etc/httpd.conf
```
Findings:
1. **Fallback is `northbeach.gg` on BOTH :80 and :443** (`000-hellbreaksim.conf`). Only two vhost
   files exist — **no `000-default.conf`**, so `--no-catch-all` will pass its guard.
2. `httpd-ssl.conf` is still Included (line 508) but declares no competing `:443` vhost. Leave it.
3. `httpd.conf:528-529` are both `IncludeOptional` (`0*` and `1*`), so the script leaves
   `httpd.conf` untouched and `100-fabsim.conf` is picked up by the existing `1*` line.
4. Both vhosts use `/opt/lampp/etc/ssl.crt/server.crt` + `/opt/lampp/etc/ssl.key/server.key`.
5. **Bare `/` is routed by `/opt/lampp/htdocs/.htaccess`**, which is generated by
   `newhost/harden-htaccess.sh` (it also forces HTTPS, so the :80 vhost only ever 301s). XAMPP's
   own `htdocs/index.php` would send an unlisted host to `/dashboard/` — hence step 3.6.
6. Database `fabsim` does not exist yet.
...
cp -a /opt/lampp/etc/httpd.conf ~/httpd.conf-$(date +%F).bak
cp -a /opt/lampp/etc/extra     ~/extra-$(date +%F).bak
sudo /opt/lampp/bin/httpd -DSSL -DPHP -S > ~/httpd-S-before-fabsim.txt 2>&1
```

## Step 3.3 — Code + database

```bash
cd /opt/lampp/htdocs/TCGEngine
git pull
grep -n "'fabsim'" Database/SiteRegistry.php                 # the registry entry must be there
grep -n -- "--no-catch-all" newhost/provision-vhost.sh | head -1

sudo DB_PASS='<real>' ./newhost/ensure-db.sh fabsim          # dry run: must say WOULD CREATE, and no registry warning
sudo DB_PASS='<real>' ./newhost/ensure-db.sh fabsim --apply
```
...
```bash
cd /opt/lampp/htdocs/TCGEngine
sudo DB_PASS='<real>' ./newhost/provision-vhost.sh fabsim \
     --server-name upf.talishar.net \
     --ports "80 443" \
     --ssl-cert /opt/lampp/etc/ssl.crt/server.crt \
     --ssl-key  /opt/lampp/etc/ssl.key/server.key \
     --no-catch-all --skip-restart
```
Expect `ok --no-catch-all: unmatched Hosts keep falling back to httpd-vhost-000-hellbreaksim.conf`.
A `!!` saying `000-default.conf still exists` means stop: that file would be the fallback.

## Step 3.6 — Bare `/` → the FaBSim menu (`.htaccess`)

`harden-htaccess.sh`'s `APP_DOMAINS` table already has the `upf.talishar.net` line (from the
`git pull` in 3.3). It **overwrites** the whole docroot `.htaccess`, so first confirm the live file
has no hand edits the script would drop:
```bash
cd /opt/lampp/htdocs/TCGEngine
sudo HTACCESS=/tmp/htaccess-preview ./newhost/harden-htaccess.sh > /dev/null   # render a preview only
diff /opt/lampp/htdocs/.htaccess /tmp/htaccess-preview   # expect ONLY the 5 added upf.talishar.net lines
sudo ./newhost/harden-htaccess.sh                        # then write the real one (backup kept)
grep -n "upf" /opt/lampp/htdocs/.htaccess
```
`.htaccess` is read per request, so no restart is needed for this step alone.

## Step 3.7 — Validate, restart, verify

```bash
sudo /opt/lampp/bin/httpd -DSSL -DPHP -t
sudo /opt/lampp/bin/httpd -DSSL -DPHP -S         # diff against ~/httpd-S-before-fabsim.txt; upf.talishar.net on :80 AND :443
sudo /opt/lampp/lampp restart
```
```bash
# 1. the existing sites and the fallback must not have moved
curl -s  -H 'Host: clarent.net'   http://127.0.0.1/TCGEngine/SharedUI/MainMenu.php | grep -io "grand archive" | head -1
curl -sk -H 'Host: northbeach.gg' https://127.0.0.1/TCGEngine/SharedUI/MainMenu.php | grep -io "hellbreak" | head -1
curl -sk -H 'Host: nope.invalid'  https://127.0.0.1/TCGEngine/SharedUI/MainMenu.php | grep -io "hellbreak" | head -1

# 2. the new site renders, over both ports (the root pointer proves ActiveSite resolves fabsim)
curl -s  -H 'Host: upf.talishar.net'  http://127.0.0.1/TCGEngine/SharedUI/MainMenu.php | grep -io "fabsim" | head -1
curl -sk -H 'Host: upf.talishar.net' https://127.0.0.1/TCGEngine/SharedUI/MainMenu.php | grep -io "fabsim" | head -1

# 3. bare "/" redirects to the menu (the .htaccess rule from 3.6)
curl -skI -H 'Host: upf.talishar.net' https://127.0.0.1/ | grep -i "^location"   # …/TCGEngine/SharedUI/MainMenu.php

# 4. through Cloudflare, once DNS is live
curl -sI https://upf.talishar.net/ | grep -iE "^HTTP|^location"
curl -sI https://upf.talishar.net/TCGEngine/SharedUI/MainMenu.php | head -1

sudo tail -40 /opt/lampp/logs/error_log /opt/lampp/logs/fabsim-error_log
```
- [ ] **Cookie isolation**: logged in on northbeach.gg → load upf.talishar.net → must be logged out.
- [ ] Create an account on upf.talishar.net and start a game, so a write lands in `fabsim`.

## Rollback

```bash
sudo rm /opt/lampp/etc/extra/httpd-vhost-100-fabsim.conf
sudo /opt/lampp/bin/httpd -DSSL -DPHP -t && sudo /opt/lampp/lampp restart
```
Remove the `upf.talishar.net` line from `harden-htaccess.sh` and re-run it (or restore its backup
from `newhost/newhost-backups-*`). Delete the `upf` DNS record. The `fabsim` database is inert without a
vhost — leave it.
